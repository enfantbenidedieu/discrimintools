...
:class:`~discrimintools.discriminant_analysis.PLSLOGIT` combines PLS Regression and Logistic Regression. It can handle multiclass
problem i.e. the target variable can have :math:`K \left(K \geq 2\right)` classes. It relies on the same principle
:class:`~discrimintools.discriminant_analysis.PLSLDA` about the number of components detection.
...
In :class:`~discrimintools.discriminant_analysis.PLSLOGIT` procedure, we can explicitly specify the number of components, with the parameter ``n_components``, for NIPALS [1]_ algorithms.
