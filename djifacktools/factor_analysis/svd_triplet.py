...
def svd_triplet(X,row_weights=None,col_weights=None,n_components=None):
    ...
    Parameters
    ----------
    X : a pandas DataFrame or an array of shape (n_rows, n_columns)
    row_weights : array with the weights of each row (None by default and the weights are uniform)
    col_weights : array with the weights of each column (None by default and the weights are uniform)
